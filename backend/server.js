...
app.get('/api/students', async (req, res) => {
  try {
    ...
    const [students] = await connection.query(
      'SELECT id, name, company, phone1, phone2, email, created_at FROM students ORDER BY created_at DESC'
    );
    connection.release();
    res.json(students);
  } catch (err) {
...
app.get('/api/students/:id', async (req, res) => {
  try {
    ...
    if (student.avatar) {
      ...
      student.avatar = 'data:image/jpeg;base64,' + student.avatar;
    }
...
app.post('/api/students', upload.single('photo'), async (req, res) => {
  try {
    ...
    if (req.file) {
      try {
        avatarBase64 = await imageToBase64(req.file.buffer);
      } catch (err) {
    ...
    const [result] = await connection.query(
      ...
      [name.trim(), company ? company.trim() : null, phone1.trim(), phone2 ? phone2.trim() : null, email.trim(), avatarBase64]
    );
    connection.release();
...
app.put('/api/students/:id', upload.single('photo'), async (req, res) => {
  try {
    ...
    // Update photo if new image is provided
    if (req.file) {
      const avatarBase64 = await imageToBase64(req.file.buffer);
      updateQuery += ', avatar = ?';
